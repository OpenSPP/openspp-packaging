...
def build_rpm():
    ...
    subprocess.check_call(['tar', 'czf', os.path.join(rpmbuild_dir, 'SOURCES', tarball),
                          '--exclude=.git', '--exclude=*.pyc', '--exclude=__pycache__', 
                          '--transform', f's,^,openspp-{VERSION}/,', '.'])
    
    # Copy spec file
    shutil.copy(os.path.join(ROOTDIR, 'setup/rpm/openspp.spec'),
                os.path.join(rpmbuild_dir, 'SPECS/'))
    
    # Build RPM
    subprocess.check_call(['rpmbuild', '-ba',
...
def build_deb():
    ...
    try:
        # Copy source
        shutil.copytree(ROOTDIR, pkg_dir, 
        ...
        if os.path.exists(changelog_file):
            ...
            changelog = changelog.replace('TIMESTAMP', time.strftime("%a, %d %b %Y %H:%M:%S +0000", time.gmtime()))
        ...
        shutil.copy(os.path.join(build_dir, deb_file), 
                   os.path.join(ROOTDIR, 'dist/'))
...
def build_windows():
    ...
    if sys.platform == 'win32':
        ...
        if not wine_path:
            logger.error("Wine not found. Install Wine to build Windows installer on Linux")
            logger.info("Install with: sudo apt-get install wine wine32 wine64")
            return
        
        # Check for NSIS in Wine
        wine_nsis = os.path.expanduser('~/.wine/drive_c/Program Files (x86)/NSIS/makensis.exe')
        wine_nsis_alt = os.path.expanduser('~/.wine/drive_c/Program Files/NSIS/makensis.exe')
...
def main():
    ...
    parser.add_argument('--build', choices=['all', 'python', 'rpm', 'deb', 'windows', 'docker'],
