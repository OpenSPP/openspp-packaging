...
def run_cmd(cmd: List[str], cwd: Optional[Path] = None, retries: int = 3, delay: int = 5) -> str:
    """
    ...
    for attempt in range(retries):
        try:
            ...
            logger.warning(f"Command '{' '.join(cmd)}' failed. Attempt {attempt + 1}/{retries}")
            logger.debug(f"Stderr: {e.stderr}")
            if attempt == retries - 1:
                raise VendorizeError(f"Command failed after {retries} attempts: {e.stderr}")
            time.sleep(delay)
...
def clone_and_checkout(repo_url: str, commit: str, dest_dir: Path, use_cache: bool = True, original_ref: str = None) -> None:
    """
        ...
        try:
            ...
            try:
                ...
                if original_ref and original_ref != commit:
                    ...
                    logger.debug(f"Fetching branch '{original_ref}' for commit {commit[:8]}")
                    try:
                        run_cmd(["git", "fetch", "--depth", "1", "origin", original_ref], cwd=dest_dir)
                        run_cmd(["git", "checkout", commit], cwd=dest_dir)
                        logger.info(f"Successfully checked out {commit[:8]} after fetching {original_ref}")
                        return
                    except VendorizeError:
                        pass
                ...
                run_cmd(["git", "checkout", commit], cwd=dest_dir)
                logger.info(f"Successfully checked out {commit[:8]} after unshallowing cache")
                return
        except VendorizeError:
    ...
    if original_ref and original_ref != commit:
        logger.debug(f"Using shallow clone of branch '{original_ref}' for commit {commit[:8]}")
        try:
            # Clone the specific branch/tag that contains our commit
            run_cmd(["git", "clone", "--depth", "1", "--branch", original_ref, repo_url, str(dest_dir)])
            # The commit should be available now since we cloned the right branch
            run_cmd(["git", "checkout", commit], cwd=dest_dir)
            logger.debug(f"Successfully checked out {commit[:8]} from branch {original_ref}")
            return
        except VendorizeError:
            logger.warning(f"Failed to clone branch '{original_ref}', falling back to default strategy")
...
def sync_from_lockfile(lockfile_data: Dict[str, Any], clean_cache: bool = False) -> None:
    """
    ...
    try:
        ...
        if 'odoo' in lockfile_data and lockfile_data['odoo']:
            ...
            clone_and_checkout(
                ...
                original_ref=odoo_config.get('ref')  # Use original ref for smart shallow cloning
            )
        ...
        for name, repo_config in lockfile_data.get('addons', {}).items():
            ...
            clone_and_checkout(
                ...
                original_ref=repo_config.get('ref')  # Use original ref for smart shallow cloning
            )
...
def main():
    ...
    parser.add_argument(
        '--sync',
        action='store_true',
        help='Sync dependencies from existing lockfile'
    )
    
    parser.add_argument(
        '--clean',
        action='store_true',
        help='Remove vendor directory'
    )
    ...
    try:
        if args.clean:
            ...
            sync_from_lockfile(lockfile_data, clean_cache=args.clean_cache if 'clean_cache' in args else False)
        
        elif args.sync:
            if not LOCKFILE.exists():
                raise VendorizeError(f"Lockfile {LOCKFILE} not found. Run --lock first.")
